...
#endif
{
	QGuiApplication::setDesktopFileName(Otter::Branding::desktopFileBaseName());
...
void FreeDesktopOrgPlatformIntegration::showNotification(Notification *notification)
{
	...
	arguments << QVariantMap({{QLatin1String("desktop-entry"), Otter::Branding::desktopFileBaseName()}, {QLatin1String("image-data"), (message.image.isNull() ? message.getIcon().pixmap(128, 128).toImage() : message.image)}});
...
void FreeDesktopOrgPlatformIntegration::setTransfersProgress(qint64 bytesTotal, qint64 bytesReceived, qint64 transfersAmount)
{
	...
	message.setArguments({QLatin1String("application://") + Otter::Branding::desktopFileName(), properties});
